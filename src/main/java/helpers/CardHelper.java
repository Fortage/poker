...
public final class CardHelper {
    ...
    public int calculatePoints(List<Card> cards) {
        ...
        {
            if (mapCards.get(card) != null
                    && mapCards.get(card+1) != null
                    && mapCards.get(card+2) != null
                    && mapCards.get(card+3) != null
                    && mapCards.get(card+4) != null)
            {
                street = mapCards.get(card).size() == 1
            ...
            if (getStreetFlash(mapCards, card) && card == 8) // ФлешРояль, тот же стритфлеш только начинается с 10ки.
            {
    ...
    private Map<Integer, List<Card>> cardToMap(List<Card> cards){
        ...
        for(Card card : cards)
        {
            if (result.get(ranksBack.get(card.getRank())) == null)
            {
                result.put(ranksBack.get(card.getRank()), Stream.of(card).collect(Collectors.toList()));
            }
            else
            {
                result.get(ranksBack.get(card.getRank())).add(card);
                result.put(ranksBack.get(card.getRank()), (result.get(ranksBack.get(card.getRank()))));
            }
        }
        return result;
    }
    ...
    private boolean getStreetFlash(Map<Integer,List<Card>> mapCard, int index){
        ...
        for (int i=0;i<4;i++)
        {
            if (mapCard.get(index) != null && mapCard.get(index+i) != null) {
                if (!mapCard.get(index + i).get(0).getSuit().equals(mapCard.get(index).get(0).getSuit())) {
                    result = false;
                    break;
                }
            }
